...
// triggerStats is called after login in applyLogin()
...
async function applyLogin(){
  ...
  if(window._applySidebarButtons) window._applySidebarButtons();
  loadFeed(); setTimeout(loadAd,3000); setInterval(loadAd, 60*60*1000); initDark(); initNotifications();
  triggerStats(); setInterval(triggerStats, 10000);
}
...
window.showStatsButtonForAdmins = function() {
    // Stats are now visible to all users - nothing to do here
    if(window._applySidebarButtons) window._applySidebarButtons();
};
...
// ====== הצגת כפתורי sidebar לפי הרשאות ======
// סטטיסטיקות - מוצגות לכולם, דיווחים - רק למנהלים ראשיים
(function initSidebarButtons() {
    function applySidebarVisibility() {
        const statsBtn = document.getElementById('statsMenuBtn');
        const reportsBtn = document.getElementById('reportsMenuBtn');
        if (statsBtn) statsBtn.style.removeProperty('display'); // visible to all
        if (reportsBtn && me) {
            reportsBtn.style.display = isSuperAdmin() ? 'flex' : 'none';
        }
    }
    // run after login
...
window.loadChatMessages = async function() {
    if (!me?.email) return;
    try {
        const res = await fetch(`${BACKEND}/chat_get?email=${encodeURIComponent(me.email)}&t=${Date.now()}`);
        const data = await res.json();
        if (data.status === 'ok') {
            const container = document.getElementById('chatMessages');
            if (!container) return;
            const msgsHtml = data.messages.map(m => `
                <div class="chat-msg ${m.email === me?.email ? 'me' : ''}">
...
window.sendChatMsg = async function() {
    ...
    try {
        const res = await fetch(`${BACKEND}/chat_send`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ email: me.email, name: me.name || 'מנהל', text: text })
        });
        const data = await res.json();
        if (data.status === 'ok') {
            input.value = '';
